...
pub trait IEscrowSrc<TContractState> {
    ...
    fn withdraw_to(
        ref self: TContractState, secret: felt252, target: ContractAddress, immutables: Immutables,
    );
...
pub mod EscrowSrc {
    ...
    impl EscrowSrc of super::IEscrowSrc<ContractState> {
        ...
        fn public_withdraw(ref self: ContractState, secret: felt252, immutables: Immutables) {
            self._only_after(immutables.timelocks.src_public_withdrawal + immutables.timelocks.deployed_at);
            self._only_before(immutables.timelocks.src_cancellation + immutables.timelocks.deployed_at);
        ...
        fn cancel(ref self: ContractState, immutables: Immutables) {
            self._only_taker(immutables);
            self._only_after(immutables.timelocks.src_cancellation + immutables.timelocks.deployed_at);

            self._cancel(immutables);
        }

        fn public_cancel(ref self: ContractState, immutables: Immutables) {
            self._only_after(immutables.timelocks.src_public_cancellation + immutables.timelocks.deployed_at);
    ...
    pub(crate) impl InternalEscrowFunctions of InternalEscrowFunctionsTrait {
        ...
        fn _only_valid_immutables(self: @ContractState, immutables: Immutables) {
            let address = self.factory.read().address_of_escrow_src(immutables);
        ...
        fn _withdraw_to(
            ...
            token.transfer(target, immutables.amount);
