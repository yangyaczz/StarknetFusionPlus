...
contract Resolver is Ownable {
    ...
    uint256 private constant _DST_NOT_EVM_FLAG = 1 << 253;
    event DstNotEVM(bytes32 indexed orderHash, address indexed taker);
    ...
    function deploySrc(
        ...
        // 添加以下检查逻辑：
        // 检查 MakerTraits 的第253位是否为1（非EVM目标链）
        uint256 makerTraitsValue = MakerTraits.unwrap(order.makerTraits);
        if ((makerTraitsValue & _DST_NOT_EVM_FLAG) != 0) {
            bytes32 orderHash = _LOP.hashOrder(order);
            emit DstNotEVM(orderHash, msg.sender);
        }
